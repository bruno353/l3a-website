...
function createPostCard({ title, content, thumbnail, link }) {
  return `<div class="col-md-4 mb-4">
            <div class="update-card">
              <div class="image-wrapper">
                <img src="${thumbnail}" />
              </div>
              <div class="p-4 d-flex flex-grow-1 flex-column">
                <h3 class="title">
                  <a href="${link}" style="text-decoration: none;" target="_blank">${title}</a>
                </h3>
                <p class="flex-grow-1" style="letter-spacing: 0; color: #555">
                  ${extractDescription(content)}...
                </p>
                <a class="mb-3" href="${link}" style="text-decoration: none;" target="_blank">
                  <img
...
async function initBlogPosts() {
  ...
  const postsStr = posts.map((post) => createPostCard(post)).join("");
